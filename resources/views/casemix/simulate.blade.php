...
<div class="row g-4 mb-4">
    ...
    <div class="col-md-4">
        <div class="card border-0 shadow-sm rounded-4 h-100 overflow-hidden bg-white transition-hover">
            <div class="card-body p-4 d-flex align-items-center gap-4">
                <div class="bg-secondary bg-opacity-10 text-secondary rounded-3 d-flex align-items-center justify-content-center flex-shrink-0" style="width: 56px; height: 56px; font-size: 1.5rem;">
                    <i class="fa-solid fa-hospital-user"></i>
                </div>
                <div>
                    <div class="small fw-semibold text-muted text-uppercase mb-1" style="font-size: 0.75rem; letter-spacing: 0.5px;">Total Biaya Riil RS</div>
                    <div class="h4 fw-bold text-dark mb-0 font-monospace">Rp {{ number_format($result['total_riil'] ?? 0, 0, ',', '.') }}</div>
                </div>
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="card border-0 shadow-sm rounded-4 h-100 overflow-hidden bg-white transition-hover">
            <div class="card-body p-4 d-flex align-items-center gap-4">
                <div class="bg-primary bg-opacity-10 text-primary rounded-3 d-flex align-items-center justify-content-center flex-shrink-0" style="width: 56px; height: 56px; font-size: 1.5rem;">
                    <i class="fa-solid fa-shield-halved"></i>
                </div>
                <div>
                    <div class="small fw-semibold text-muted text-uppercase mb-1" style="font-size: 0.75rem; letter-spacing: 0.5px;">Plafon INA-CBG</div>
                    <div class="h4 fw-bold text-primary mb-0 font-monospace">Rp {{ number_format($result['tarif_ina_cbg'] ?? 0, 0, ',', '.') }}</div>
                </div>
            </div>
        </div>
    </div>
    <div class="col-md-4">
        ...
        @php
            ...
            $textColor = $p > 100 || $p <= 85 ? 'text-white' : 'text-dark';
            $textMutedColor = $p > 100 || $p <= 85 ? 'text-white-50' : 'text-muted';
        @endphp
        <div class="card border-0 shadow-sm rounded-4 h-100 overflow-hidden {{ $bgClass }} {{ $textColor }} position-relative transition-hover">
            ...
            <div class="card-body p-4 d-flex align-items-center justify-content-between position-relative z-1">
                <div>
                    <div class="small fw-semibold {{ $textMutedColor }} text-uppercase mb-1" style="font-size: 0.75rem; letter-spacing: 0.5px;">Persentase Utilisasi</div>
                    <div class="h2 fw-bold mb-0 font-monospace">{{ number_format($p, 1, ',', '.') }}%</div>
                </div>
                <div class="bg-white bg-opacity-25 rounded-3 d-flex align-items-center justify-content-center flex-shrink-0" style="width: 56px; height: 56px; font-size: 1.25rem;">
            ...
            <div class="card-body p-4 d-flex gap-3 align-items-center">
                ...
                <div class="flex-grow-1">
                    <div class="d-flex align-items-center gap-3 mb-1">
                        <span class="h5 fw-bold text-{{ $alertClass }} mb-0 font-monospace">{{ $result['kode_inacbg'] ?? 'TARIF BELUM TERSEDIA' }}</span>
                        <span class="badge bg-{{ $alertClass }} px-3 py-1 rounded-pill fw-semibold" style="font-size: 0.7rem; letter-spacing: 0.5px;">
                            {{ strtoupper($result['status'] ?? 'UNKNOWN') }}
                        </span>
                    </div>
                    <div class="text-{{ $alertClass }} opacity-75 fw-medium small lh-sm">{{ $result['pesan'] ?? $result['message'] ?? 'Lengkapi rincian billing dan diagnosis klinis untuk melakukan simulasi tarif INA-CBG.' }}</div>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="p-4 border-bottom border-light d-flex justify-content-between align-items-center bg-white">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-muted small mb-0 fw-medium">Total: {{ count($encounter->billingInvoice?->billingDetails ?? []) }} Item Layanan</p>
                    </div>
                </div>
            </div>
                ...
                <table class="table table-hover table-borderless align-middle mb-0 custom-table">
                    <thead class="bg-light">
                        <tr class="text-muted small fw-semibold text-uppercase" style="letter-spacing: 0.5px;">
                            <th class="ps-4 py-3 rounded-start">Kategori Layanan</th>
                            <th class="py-3">Deskripsi Tindakan/Obat</th>
                            <th class="py-3 text-center">Jumlah (Qty)</th>
                            <th class="py-3 text-end">Harga Satuan</th>
                            <th class="pe-4 py-3 text-end rounded-end">Total Biaya</th>
                        </tr>
                    </thead>
                        ...
                        <tr>
                            <td colspan="5" class="text-center py-5">
                                <div class="d-inline-flex align-items-center justify-content-center bg-light rounded-circle mb-3 shadow-sm" style="width: 80px; height: 80px;">
                                    <i class="fa-solid fa-file-invoice fs-2 text-muted opacity-50"></i>
                                </div>
                                <h6 class="fw-bold text-dark mb-1">Rincian Kosong</h6>
                                <p class="text-muted small mb-0">Rincian billing belum tersedia untuk kunjungan ini.</p>
                            </td>
                        </tr>
                    @endforelse
                    </tbody>
                    ...
                    <tfoot class="bg-light bg-opacity-50">
                        <tr>
                            <td colspan="4" class="text-end py-4 fw-semibold text-muted text-uppercase" style="font-size: 0.85rem; letter-spacing: 0.5px;">Grand Total Biaya Riil</td>
                            <td class="pe-4 text-end py-4 h5 mb-0 fw-bold text-primary font-monospace">Rp {{ number_format($result['total_riil'] ?? 0, 0, ',', '.') }}</td>
                        </tr>
                    </tfoot>
                    @endif
                </table>
...
<div class="mt-4 d-flex flex-column flex-sm-row justify-content-between gap-3">
    <a href="{{ route('casemix.index') }}" class="btn btn-light border border-light text-muted fw-bold px-4 py-2 shadow-sm rounded-pill transition-hover hover-bg-gray">
        <i class="fa-solid fa-arrow-left me-2"></i>Kembali ke Monitoring
    </a>
    <button class="btn btn-primary shadow-sm px-4 py-2 fw-bold rounded-pill transition-hover" onclick="window.print()">
